...
# select only phase 1
phase1Files<- read.csv2("premup-flat/outputs/group_level/group_task-learning.csv",sep=";",
                        header=T,stringsAsFactors=FALSE)


# phase 2
phase2Files<- read.csv2("premup-flat/outputs/group_level/share/group_task-rec.csv",sep=";",
                        header=T,stringsAsFactors=FALSE)
...
for(mod in winMod){
  ...
  for (j in 1:length(participants)){
    tryCatch({
      ...
      file2<-phase2Files[phase2Files$participant==SubNum,]
