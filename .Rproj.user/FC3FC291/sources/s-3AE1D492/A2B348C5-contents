# -----------------------------------------------------------------------------#
# Parameter recovery functions for the Evaluative Model with a free learning
# rate
#
...
source(("likelihood_functions/lik_RW_feedb.R"))
source(("fitting_functions/fit_RW_feedb.R"))
source(("simulation_functions/simulate_RW_feedb.R"))
...
dat<-foreach (j=1:sims, .combine=rbind,.packages=c('pracma', 'here'))  %dopar% {
  # simulate data
  sim<-simulate_RW_feedb(T = trials, mu =  mu, alpha =  alpharan[j],
  ...
  est<-searchGlobal(data = sim, alphaBound = alphaBound, betaBound = betaBound, 
                    ...
                    fittingfunction = fit_RW_feedb , model = "feedb" ) 
