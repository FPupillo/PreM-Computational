lik_RescorlaWagner_feedbAndObs<-function (Data,alpha, beta, omega, print,  initialQ, initialQ_o){
  ...
  #   omega<-weight assigned to each of the two model updating
  ...
  for (i in 1:ncol(initialQ)){
    ...
    Delta_index[i]<-paste("Delta", i, sep="")
  ...
  # assign information about the category that participants' chose
  y<-getfeedb(Data, initialQ)
  # loop over trials
  for (t in 1: max(Data$trialN)){
    ...
    #p<-exp(beta*Q+(1-beta)*Q_o)/sum(exp(beta*Q+(1-beta)*Q_o))
    
    m<- omega*Q +(1-omega)*Q_o
    ...
    if (Data$response[t]!=0){
      ...
      # the feedb is a vector of 0, used to update the model
      feedb<-rep(0, ncol(initialQ))
      
      delta <-feedb-Q # subtracting 1 to the objectcategory, the category participants predicted at the end
