...
for(mod in winMod){
  ...
  # get phase 1 files
  phase1Files<- read.csv(paste("computational_model/output_files/fittedData.",setup, ".Phase1.",
                                mod, ".csv", sep=""))
  ...
  for (j in 1:length(participants)){
    ...
    # subset phase 1
    Phase1sub<-phase1Files[phase1Files$participant==participants[j],]
    
    # initialize variables
    Qindex<-c("Q1", "Q2")
    
    # initialize matrix  
    matrixQ<-matrix(NA, nrow = 4 ,ncol = 2)
    for (s in 1:4){
        matrixQ[s,]<-unlist(tail(Phase1sub[Phase1sub$scene_cat==s,Qindex],1))
    }
    #---------------------------------------------------------------------------------------------#
