simulate_fLR_Eval<-function(
  ...
  for ( s in 1:sceneNum){
    ...
    object_cat<-sample(c(1:ncol(mu)), T, prob=mu[s,], replace=T)
    ...
    for(t in 1:T){
      ...
      updateVal<-update(r = r, Q = Q[a[t]], alpha = alpha)
    ...
    preference<-NA
    for (e in 1 :length(a)){ 
      if(a[e]==which(mu[s]==max(mu[s]))){choice[e]<-1
      }else{choice[e]<-0}
