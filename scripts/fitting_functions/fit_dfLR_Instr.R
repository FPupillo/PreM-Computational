fit_dfLR_Instr<-function(data,alphaBound, betaBound, initialQ){
  ...
  X0<-c( runif(1),  rexp(1,1))  # rexp generates random numbers from the exponential 
  #  distributon with mean 1
  LB<-c(alphaBound[1],betaBound[1]) # lower boundary
  UB<-c(alphaBound[2], betaBound[2]) 

  
  obfunc<-function(x) lik_dfLR_Instr(data, x[1] , x[2], 1, initialQ) # this function 
