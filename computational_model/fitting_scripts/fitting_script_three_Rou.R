...
setwd("computational_model/likelihood_functions_Rou")
likfun<-list.files()
for (f in 1:length(likfun)){
  source(likfun[f])
}
# fitting functions
setwd(paste(cd, "/computational_model/fitting_functions_Rou",sep=""))
fitfun<-list.files()
...
mod<-"RescorlaWagner_absPEchoiceLR"
# loop over mod
...
for(mod in mod){
  ...
  filename<-paste("premup-", setup, "/outputs/group_level/computational_model",
                  "/ParameterEstimation.betalimit=30.initialQ=0.5.", mod, ".csv", sep="")
  ...
  for (j in 1:length(participants)){
    ...
    data1<-likelihoodfunction(Data=currfile1, beta=beta, eta=eta, k=k,
    ...
    fitdata<-likelihoodfunction(Data = currfile, beta = beta, eta=eta, k=k,
                                  print =3, initialQ =  initialQ)
