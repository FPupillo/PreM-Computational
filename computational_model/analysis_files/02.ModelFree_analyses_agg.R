...
# plot as in premup
plot1<-ggplot(exp1, aes( x=contingency, y=id_acc))+
  geom_bar(aes(contingency, id_acc, fill = contingency),
...
exp2$predicted_contingency<- ifelse(exp2$predicted_contingency=="0,5", "0.50", 
                                   ifelse(exp2$predicted_contingency=="0,1" , "0.10",
                                          ifelse(exp2$predicted_contingency=="0,9" , "0.90",
...
VoI1<-c("participant", "prediction_condition","predicted_category", 
       "predicted_contingency","enc_acc", "id_acc")
...
VoI2<-c("participant", "prediction_condition", "predicted_category", 
        "predicted_contingency", "trial_acc", "id_acc")
...
# analyse
modexp1_pred_cont<-glmer(recognition_accuracy~prediction_accuracy*predicted_contingency+
                          (prediction_accuracy*predicted_contingency  | participant),
                        family = binomial, data = exp1, glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))

summary(modexp1_pred_cont)

Anova(modexp1_pred_cont)

# only accurate
modexp1_pred_cont_corr<-glmer(recognition_accuracy~predicted_contingency+
                               (predicted_contingency  | participant),
                             family = binomial, data = exp1[exp1$prediction_accuracy=="Correct",]
                             , glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
summary(modexp1_pred_cont_corr)




# only inaccuractee
modexp1_pred_cont_incorr<-glmer(recognition_accuracy~predicted_contingency+
                                (predicted_contingency  | participant),
                              family = binomial, data = exp1[exp1$prediction_accuracy=="Incorrect",]
                              , glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
summary(modexp1_pred_cont_incorr)

lsmeans(modexp1_pred_cont, pairwise~prediction_accuracy*predicted_contingency, 
        adjust = "bonferroni")
...
exp2$prediction_accuracy<-ifelse(exp2$prediction_accuracy==0, 
                                 "Incorrect", "Correct")
##---------------------------------------------------------
