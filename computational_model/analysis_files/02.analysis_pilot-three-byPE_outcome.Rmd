...
#     scale_fill_viridis_d()
  ...
  #ggsave(paste0("/home/francesco/PowerFolders/Frankfurt_University/PIVOTAL/", "PREMUP_computational/computational_model/figures/PE_instr_distrNOscene.exp1.exp2.png"), 
      #notate(geom="text",  label="b",size=8,family="serif")+
  #guides(fill=guide_legend(title="Scene Condition"))+
...
# analyse
PEbyAccAll<-glmer(id_acc~PEchoice*alpha*exp+(PEchoice|SubNum), family= binomial, 
                   data = allData,
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))

summary(PEbyAccAll)

Anova(PEbyAccAll, type = "II")


# we need to center PE
allData$PEchoice.c <- scale(allData$PEchoice, center=TRUE, scale=FALSE)
...
# plot it again
print(
  ggplot(allData, aes( x=PEchoice.c, y=id_acc))+
    geom_line(stat="smooth",method = "glm", formula=y~x,method.args=list(family="binomial"), alpha=0.5)+
    aes(colour = factor(SubNum), alpha=0.3)+
    geom_smooth(method="glm",formula=y~x,method.args=list(family="binomial"), colour="black", se=T)+
    theme(strip.text.x = element_text(size = 13))+ ylim(c(0,1))+
...
# in the positive side
PEbyAccAllPos<-glmer(id_acc~PEchoice.c+(PEchoice.c|SubNum), family= binomial, 
                      data = allData[allData$trial_acc==1,],
                      control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))

summary(PEbyAccAllPos)

# in the negative side
PEbyAccAllNeg<-glmer(id_acc~PEchoice.c+(PEchoice.c|SubNum), family= binomial, 
                      data = allData[allData$trial_acc==0,],
                      control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
...
# create absolute PE
allData$absPEchoice.c<-abs(allData$PEchoice.c)

# analyse abs 
absPEchoice<-glmer(id_acc~absPEchoice.c+(absPEchoice.c|SubNum), family= binomial, 
                   data = allData,
                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
...
### Analysis with outcome-based PE

```{r}
# get the outcome based PE
#create the function


allDataOutPE<-data.frame(rbind(exp1,exp2 ))
