...
name<-paste("exp1/outputs/group_level/computational_model/ParameterEstimation.exp1_learning.", 
...
Param<-foreach (j=1:length(phase1Files),.combine=rbind
                #.packages='pracma'
                )  %dopar% {
  ...
  tryCatch({
    ...
    start_time<-Sys.time()
    est<-searchGlobal(data = file1,alphaBound= alphaBound,betaBound= betaBound, 
