lik_RescorlaWagner_feedbALLdualLR<-function (Data,alphapos,alphaneg, beta,print,  initialQ){
...
# assign information about the feedb
...
for (t in 1: max(Data$trialN)){
  ...
  if (Data$response[t]!=0){
    ...
    delta<-Data[t,xindex]-Q
    
    # get the learning rate depending on PE
    # if prediction error is bigger than 0, positive learning rate
    if (delta[Data$response[t]]>=0){
      lr<-alphapos
    }else{
      lr<-alphaneg
    }
    ...
    # update values
    updateVal<-update(r = Data[t,xindex], Q = Q, alpha = lr)
    ...
    # update the values, considering that the feedback is 0
    updateVal<-update(r = c(0,0,0) , Q = Q, alpha = alphaneg)
