lik_RescorlaWagner_obsALLabs<-function (Data,alpha, beta,print,  initialQ){
...
# even when there is no response. We consider the absolutePE
#
# Input
...
for (t in 1: max(Data$trialN)){
  ...
  delta <-abs(Data[t,xindex]-Q)/sum(delta) # subtracting 1 to the objectcategory, the category shown at the end
