...
fit_RescorlaWagner_obsALLnobeta<-function(data,alphaBound, initialQ){
  ...
  X0<-c(runif(1)  )# rexp generates random numbers from the exponential distributon with mean 1
  LB<-c(alphaBound[1]) # lower boundary
  UB<-c(alphaBound[2]) 

  # this function is similar to the MATLAB "handle" function
  obfunc<-function(x) lik_RescorlaWagner_obsALLnobeta(data, x[1],1, initialQ) 
